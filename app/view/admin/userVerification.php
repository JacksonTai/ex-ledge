...
<body>
                         ...
                         <div class="user-verification-wrapper" id="<?php echo htmlspecialchars($user['user_id']) ?>" data-user-id=<?php echo htmlspecialchars($user['user_id']) ?>>
                              <div class="user-verification dialog">
                                   ...
                                   <div class="user-verification-btn-container">
                                        <button class="user-verification-btn user-verification-btn--accept" name="accept">Accept</button>
                                        <button class="user-verification-btn user-verification-btn--reject" name="reject">Reject</button>
                                   </div>
     ...
     <?php
          ...
          if(isset($_POST['accept'])) {
               ...
               $sql = "UPDATE user SET verification = '1' WHERE user_id = '$user[user_id]'";
               
               $result = mysqli_query($conn, $sql);  
               
               if ($result) { 

                    $delete_verif = "DELETE FROM verification_queue WHERE user_id = '$user[user_id]'";
                    $delete_verif_result = mysqli_query($conn, $delete_verif);

                    echo "<scriptalert('successful')</script>"; 
               } else {
          ...
          if(isset($_POST['reject'])) {
               ...
               $sql = "DELETE FROM verification_queue WHERE user_id = '$user[user_id]'";
